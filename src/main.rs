...
use db_utils::utils::{get_pool, run_migrations};
...
async fn get_published(state: Data<AppState>) -> impl Responder{
    ...
    match db.send(GetArticles).await{
        ...
        _ => HttpResponse::InternalServerError().json("Some went wrong"),
    }
}

#[actix_web::main]
async fn main() -> std::io::Result<()>{
    let db_url = env::var("DATABASE_URL").expect("Error with retrieving the database URL");
